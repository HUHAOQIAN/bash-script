...
input_user_info(){
    ...
    case $input in
        [yY][eE][sS]|[yY])
            echo "继续安装"
            export graffiti=$name
            export mail=$mail
            echo "保存用户信息到user_info文件"
            ;;
...
mintAsset(){
    ...
    echo "name=$graffiti"
    echo "mint amount=1000"
    
    docker exec -it node bash -c "ironfish wallet:mint --metadata="$graffiti" --name=$graffiti --amount=1000 --account=$graffiti --fee=0.00000001"

}

burnAsset(){
    echo y | docker exec -it node bash -c "ironfish wallet:mint --metadata="$graffiti" --name=$graffiti --amount=1000 --account=$graffiti --fee=0.00000001 | tee Asset.txt" 
    Asset=$(awk '/Asset Identifier: /{print $3}' Asset.txt)
    echo "获取的资产标识：$Asset"
    sleep 600
    echo "正在燃烧资产...."
    docker exec -it node bash -c "ironfish wallet:burn --assetId=$Asset --amount=1000 --account=$graffiti --fee=0.00000001"
}

transferAsset(){
    echo "正在转账资产到官方地址...."
    docker exec -it node bash -c "ironfish wallet:send --to dfc2679369551e64e3950e06a88e68466e813c63b100283520045925adbe59ca --amount 2 --fee 0.00000001 --account $graffiti --memo "$graffiti" "
}
...
欢迎关注，如有收费请勿上当受骗。
 ...
 ${Green_font_prefix} 18.保存昵称和graffiti到user_info文件${Font_color_suffix}
 ———————————————————————" && echo
